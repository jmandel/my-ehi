...
|---|---|---|---|
| `IMMUNE` | **Spine (EPT side):** per-dose ledger, PK `IMMUNE_ID` | 19 | Carries order/charge/encounter linkage (`ORDER_ID`, `IMM_CHARGE_REC_ID`, `IMM_CSN`), `IMMNZTN_ID`→masterfile, status (all `Given` here), `IMM_HISTORIC_ADM_YN`, `EXTERNAL_ADMIN_C_NAME`. **No PAT_ID column.** |
...
| `IMMUNE_HISTORY` | Change-audit / versions of `IMMUNE` (§17), PK `(IMMUNE_ID, LINE)` | 30 | `HX_`/`HIST_`-flavored columns (`IMM_TYPE_HIST_ID`, `IMMNZTN_HX_DATE`…); `LINE` increments per edit. 19 immunizations → 30 versions. |
...
- **EPT ledger → audit:** `IMMUNE_HISTORY.IMMUNE_ID = IMMUNE.IMMUNE_ID`, replay by `LINE` (§17, §20).
...
- **DXR/EPT → type masterfile:** `*.{IMM_TYPE_ID | IMMUNZATN_ID | IMM_COMP_GROUP_ID | IMM_DUE_TYPE_ID} =
  CLARITY_IMMUNZATN.IMMUNZATN_ID` (§3). One shared masterfile across all four tables; every populated id
  resolves. A denormalized `_NAME` companion (`IMM_TYPE_ID_NAME`, `IMMUNZATN_ID_NAME`, `IMM_DUE_TYPE_ID_NAME`)
  sits beside each id, so you usually don't *need* the join for display (§4).
- **DXR document is NOT a scanned-media document.** The immunization `DOCUMENT_ID`s (`37763216`, …) do **not**
  ...
  masterfile" is its own id space, distinct from the Media/scanned-document space (§24).

## Unstructured tie-back

n/a for RTF/Media — no `Rich Text/*.RTF` or `Media/*` file is referenced by any immunization table, and
`IMM_NOTES`/`IMM_NOTES_RAW_DATA` are empty here. Human-entered text lives **inline** in paired `_FT` /
`FREE_TEXT` columns instead of separate notes (§14): `IMM_TYPE_FREE_TEXT` (e.g. brand/vial text),
`IMM_VALID_RSN_FT` and `IMM_COMP_VALID_RSN_FT` (free-text schedule-validity explanations like "the client's
age and vaccination history allowed for certain doses in the series to be skipped"), `IMM_DUE_TYPE_FT`
...
1. **`IMM_ADMIN` row count wildly overstates shots — it's re-extracted snapshots, not events.**
   ...
   **composite Document** that Epic **re-extracts in full on every contact** that touches immunizations,
   producing a cumulative snapshot keyed by `CONTACT_DATE_REAL`. Line counts grow monotonically (1 line in
   8/2018 → 6 by 1/2020 → 18 by 12/2025). This is pattern §19 (per-contact re-snapshot) applied to a document.
   *Handle:* take the **latest `CONTACT_DATE_REAL`** of the master document as the truth; never `COUNT(*)`
   over all of `IMM_ADMIN`. Use `ORDER BY CAST(CONTACT_DATE_REAL AS REAL)` — the text `CONTACT_DATE` sorts
   lexically and lies (§10).

2. **There is more than one DXR document — outside-pharmacy records sit in their own documents.**
   *Observe:* `IMM_ADMIN` has three `DOCUMENT_ID`s (here `37763216` with 118 rows/15 snapshots, plus
   `37596260` 5 rows and `151107765` 1 row). *Mechanism:* the small documents are **externally-sourced**
   administrations (their `IMM_LOCATION` reads "CVS #04930", "WALGREENS #06130") captured as separate DXR
   documents before/while being reconciled into the master document. `IMM_DUE` uses only the master document.
...
3. **`IMM_REFERENCE_ID` carries an embedded `0x7F` (DEL) byte — literal string matches silently fail.**
   *Observe:* `LENGTH(IMM_REFERENCE_ID)` returns 17 for a value that displays as 16 digits; `WHERE
   ...
   '') = '6647463431000071'`. `hex()` reveals the `7F`.
...
7. **`IMM_DUE_TYPE_ID` can be blank with the real label only in `_FT`.** *Observe:* an `IMM_DUE` row with
   ...
   masterfile record, so Epic stores the free-text label (§14). *Handle:* `COALESCE(IMM_DUE_TYPE_ID_NAME,
   IMM_DUE_TYPE_FT)` when listing due vaccines.
...
WHERE a.DOCUMENT_ID = '37763216'
  AND CAST(a.CONTACT_DATE_REAL AS REAL) =
      (SELECT MAX(CAST(CONTACT_DATE_REAL AS REAL)) FROM IMM_ADMIN WHERE DOCUMENT_ID='37763216')
...
- **Orphan bridge lines.** `PAT_IMMUNIZATIONS` LINE 7/8 point at `IMMUNE_ID`s (71612083, 71612085) that have
  **no `IMMUNE` row** — soft-deleted/superseded doses whose detail wasn't extracted (§18). Detail is not
...
- **Provenance columns empty.** `IMM_VFC_ELIGIBILITY_STATUS_C_NAME`, `IMM_FUNDING_SOURCE_C_NAME`,
  `IMM_EXT_ADMIN_C_NAME`, `IMM_FILTER_RSN_C_NAME`, `IMM_EVENT_IDENT`, `IMM_EXTERNAL_IDENTIFIER` are all blank
  here, so the registry/Care-Everywhere reconciliation provenance couldn't be confirmed from values — they are
  the fields to check on an export that *does* pull from a state registry.
