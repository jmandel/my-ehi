...
- A `ZC_RESULT_FLAG` / `ZC_ORDER_STATUS` lookup — **not shipped**; categories arrive pre-resolved as
  `*_C_NAME` (§13). This org ships labels inline, no `ZC_` tables.
...
- **`ORDER_NARRATIVE.ORDER_PROC_ID = ORDER_PROC.ORDER_PROC_ID`**, reassemble with
  `ORDER BY CAST(LINE AS INT)` (LINE is text; without the cast it sorts 1,10,11,…,2 — §7). *Verified: COVID
...
- **`ORDER_PROC_2/3/4` left-join on `ORDER_PROC_ID`** (§6) — 1:1, all 42 rows, for collection/review detail.
...
- The `ORDER_RESULTS.RESULT_VAL_START_LN/END_LN` and `RESULT_CMT_START_LN/END_LN` columns are line-pointer
  slots into a result text blob; **empty in this specimen** — the text is instead in `ORDER_NARRATIVE` /
  `ORDER_RES_COMMENT` keyed by `ORDER_PROC_ID`. Don't rely on these pointers.
...
1. **An order count massively overstates resulted labs (the placement-vs-result order split).**
   *Observe:* 42 `ORDER_PROC` rows, 24 of them labs, but only **10 distinct orders** have any
...
2. **`9999999` is the numeric sentinel for a qualitative result (§21).**
...
3. **Abnormal flag and in-range flag are complementary, not co-populated.**
   *Observe:* a normal component has `RESULT_IN_RANGE_YN = 'Y'` and an **empty** `RESULT_FLAG_C_NAME`; an
   ...
   treat `'(NONE)'`/empty as normal. Order-level rollup lives in `ORDER_PROC.ABNORMAL_YN = 'Y'` when any
   component is abnormal.

4. **"Final result" orders with ZERO structured results.**
   *Observe:* `COMPREHENSIVE METABOLIC PANEL`, both `CBC WITH DIFFERENTIAL`, and `TROPONIN` are
   `LAB_STATUS_C_NAME = 'Final result'` in `ORDER_PROC` yet have **no** `ORDER_RESULTS` rows and **no**
   narrative — their numeric values are simply absent from the structured export. *Why:* these were
...
6. **Three time flavors on a result; only the instants are real clock times (§11).**
   *Observe:* `RESULT_DATE` always renders at `12:00:00 AM` (effective/contact date). The real moments are
   ...
   §10) — its `.01` fraction even tells you *which* same-day contact resulted it — and read `COMP_*_INST_TM`
...
7. **LOINC is present only as an opaque internal id.**
   *Observe:* `COMPON_LNC_ID` (e.g. SODIUM = 21291) is populated on 41/47 rows with
   ...
   schema was **not exported** (§5). *Handle:* you cannot recover the actual LOINC (`2951-2`) from this
...
- **Where do the missing CMP/CBC/Troponin numerics live?** In this specimen they're absent from structured
...
- **`RESULT_SUB_IDN`** = 1 on the Hep C row; it's the sub-identifier slot for multi-specimen/multi-organism
  micro results. Trivial (always 1) in this record — but in a heavier micro/culture export expect it to
  enumerate organisms/specimens, with `ORGANISM_SNOMED_CT`/`ORGANISM_QUANTITY` columns becoming meaningful.
- **`OBS_MTHD_ID` and the method payload** were stripped to a bare index here (§5) — a reminder the exported
