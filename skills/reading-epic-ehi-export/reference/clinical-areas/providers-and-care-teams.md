...
| `CLARITY_DEP_4` | DEP supplement (1:1 on `DEPARTMENT_ID`) | 16 | Holds `DEP_TYPE_C_NAME` (Internal/External), `MED_REC_STYLE_C_NAME`. Note the family skips `_2`/`_3` — only `_4` ships (see §6). |
| `PAT_PCP` | **PCP / care-team history over time** (patient-level) | 2 | Line-numbered. `PCP_PROV_ID`→SER, `PCP_TYPE_C_NAME`, `SPECIALTY_C_NAME`, `EFF_DATE`/`TERM_DATE`, plus `RELATIONSHIP_C_NAME` + `OTHER_*` free-text fields for non-Epic care-team members. |
| `TREATMENT_TEAM` | **Per-encounter care-team members** | 11 | Keyed by CSN. `TR_TEAM_ID`→SER provider, `TR_TEAM_REL_C_NAME` (role: Consulting Physician, Nurse Practitioner…), `TR_TEAM_SPEC_C_NAME`, `TR_TEAM_BEG/END_DTTM`. One row per team member per contact. |
...
| `EPT_CARE_TEAMS` | **Longitudinal patient care-team** ("Provider Care Team" master) | **EMPTY** | *(from schema doc)* keyed by CSN, points to a `CARE_TEAMS_ID` (a Care-Team **group** id — a *different* id space from SER, not a provider). Would hold named provider-care-teams when configured. |
| `CLARITY_LWS` / `CLARITY_LWS_3` | Workstation/kiosk masters | 1 / 1 | Not providers — resolves `*_WORKSTATION_ID` (where an action happened). Listed so you don't confuse them with people. |
...
- **Any provider id → name:** `<anything>.<X>_PROV_ID = CLARITY_SER.PROV_ID` → `PROV_NAME` / `EXTERNAL_NAME`.
  ...
  `CLARITY_SER`. There are **51 populated tables** carrying at least one `*_PROV_ID` column (different
  prefixes — `BILLING_PROV_ID`, `AUTHRZING_PROV_ID`, `ORD_PROV_ID`, `ATTEND_PROV_ID`, `REFERRING_PROV_ID`,
  `PERFORMING_PROV_ID`, `SUP_PROV_ID`, …); **every one resolves to `CLARITY_SER.PROV_ID`** (master-file
  FK convention, §3). The column *prefix* tells you the provider's *role* in that row; the master is always
  the same.
- **Any user id → name:** `<anything>.<X>_USER_ID = CLARITY_EMP.USER_ID` → `NAME`. Verified
  ...
  `*_PROV_ID` = *the clinical actor of record*. Different question, different master (§24).
...
- **PCP, point-in-time vs current:** `PATIENT.CUR_PCP_PROV_ID` is the *current* PCP (single value).
  ...
  current (see §16/effective-dating). Verified: line 1 DHILLON (termed 8/28/2022), line 2 RAMMELKAMP
...
- **Care team for a visit:** `TREATMENT_TEAM.PAT_ENC_CSN_ID = PAT_ENC.PAT_ENC_CSN_ID`, then
  ...
  single `VISIT_PROV_ID` rendering provider on `PAT_ENC` (see §2 CSN, §7 LINE child rows).
...
1. **Provider ids and user ids are two different ID spaces for the *same human*.** Aaron Gilmour is
   ...
   `*_USER_ID` through `CLARITY_EMP` — never cross them. (This is §24 made concrete; tell SER ids from EMP
...
2. **SER provider `_PROV_NAME` companions are deliberately *dropped*, but EMP `_NAME` companions ship.**
   ...
   **Handling:** for providers, **always join `CLARITY_SER` yourself**; for users, you can usually read the
   inline `*_USER_ID_NAME` (but `CLARITY_EMP` is the fallback). This asymmetry refines patterns §4/§5 — the
...
3. **The master's own key column is renamed vs its schema doc.** `_schema_column` lists `CLARITY_SER`
   ordinal 1 as `PROV_ID_PROV_NAME`, but the real column is plain `PROV_ID`. Same flavor on `CLARITY_DEP`.
   **Mechanism:** the doc documents the master under its public-view (name) projection; the export ships
   the raw-key form. **Handling:** confirm with `PRAGMA table_info` (§5) — the real cols are
...
4. **Sentinel / placeholder "providers" and "departments" inflate the masters.** `CLARITY_SER` carries
   ...
   **imported from outside the Epic instance** — when you see `8800099`/`8`, the encounter/result came from
   ...
   providers or visits; treat `8800099`/`8` as an "external origin" marker. In this specimen
...
7. **NULL provider *and* NULL department travel together on non-face-to-face contacts.** ~56% of
   ...
   MyChart messages, telephone, and system/automated contacts (§26) — real CSNs with no rendering
...
ORDER BY tt.TR_TEAM_BEG_DTTM DESC;
...
- **`EPT_CARE_TEAMS` (the longitudinal "Provider Care Team" table) is empty.** Its `CARE_TEAMS_ID` is a
