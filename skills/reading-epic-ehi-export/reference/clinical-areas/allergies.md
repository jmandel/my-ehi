...
| `PAT_ALLERGIES` | **EPT→LPL bridge.** `(PAT_ID, LINE, ALLERGY_RECORD_ID)`. The only place the patient is tied to their allergies. | 5 | More rows than `ALLERGY` — holds deleted/superseded allergy ids too (orphan, see Gotchas). |
...
- **No `ZC_ALLERGY_REACTION` / `ZC_ALLERGY_SEVERITY` / `ZC_*` lookups at all** — this export ships **zero**
  `ZC_` tables; every category is pre-resolved to `_C_NAME` inline (genre variation, see general-patterns §13).
...
- **Patient → allergies (the only path):** `PAT_ALLERGIES.PAT_ID = PATIENT.PAT_ID` and
  `PAT_ALLERGIES.ALLERGY_RECORD_ID = ALLERGY.ALLERGY_ID`. `ALLERGY` has **no `PAT_ID` column** — you cannot
  go patient→allergy without this bridge. Verified: 4 of 5 `PAT_ALLERGIES` rows resolve to `ALLERGY`; LINE 1
  (`30689231`) is an orphan.
- **Allergy → reactions:** `ALLERGY_REACTIONS.ALLERGY_ID = ALLERGY.ALLERGY_ID`, ordered by `LINE`
  (§7 LINE child rows). Coded reaction is `REACTION_C_NAME` (all "Hives" here).
- **Allergy → allergen master:** `ALLERGY.ALLERGEN_ID = CL_ELG.ALLERGEN_ID`. Verified: all 4 resolve. But
  the name is **already denormalized** onto the row as `ALLERGY.ALLERGEN_ID_ALLERGEN_NAME` (§4), so you
...
- **Allergy → encounter (recent only):** `ALLERGY.ALLERGY_PAT_CSN = PAT_ENC.PAT_ENC_CSN_ID` (§2 CSN). Verified:
...
2. **Reaction is coded in `_C_NAME` but the schema tells you to "join the ZC table" that doesn't exist.**
   ...
   ships **no `ZC_` tables at all** (general-patterns §13 — the big genre variation). *Handle:* read the label
...
3. **`PAT_ALLERGIES` has more rows than `ALLERGY` — the extra is a hard-deleted allergy.** Here LINE 1
   ...
   *Why:* `ALLERGY` ships only *current/active* allergies (`ALRGY_STATUS_C_NAME` is "Active" for all 4); when
   ...
   record is dropped from the export — Chronicles' usual soft-delete (general-patterns §18) but with the
...
7. **`PATIENT_ALG_UPD_HX.ALRG_UPDT_TIME` shows date `1/1/1900`.** *Why:* it's a **time-only** column on
   Epic's time epoch (1/1/1900) — only the clock part is meaningful (general-patterns §11). The real instant
...
SELECT pr.PAT_ENC_CSN_ID, pr.CONTACT_DATE,
       ...
       pr.PAT_REVIEW_ELG_R_YN AS response   -- Y=confirm, N=reject, ''=shown only
...
- `PAT_REVIEW_ALLERGI` rows with a *blank* `PAT_REVIEW_ELG_R_YN` (e.g. the 4/23/2025 review, CSN 1127808563)
  appear to record allergens *shown* without a recorded Y/N response — likely a review opened but not
  answered. Unconfirmed whether blank ever means "implicitly confirmed."
