...
encounter the section is reviewed** (§19), so the same facts repeat across many CSNs.
...
| `MEDICAL_COND_INFO` | Tiny no-add lookup of medical-condition names. | 2 | `MEDICAL_COND_ID` → `MEDICAL_COND_NAME`. Peripheral. |
...
a `*_REVIEW_*` attestation table accompany each history section.
...
- **History row → its encounter contact:** `<HX>.PAT_ENC_CSN_ID = PAT_ENC.PAT_ENC_CSN_ID`. Verified all 8
  ...
  contact*, fraction `.01`/`.02` (see next). (see §2 CSN/contacts, §10 `*_DATE_REAL`)
...
- **Family condition → the relative it's about:** `FAMILY_HX.FAM_MED_REL_ID = FAMILY_HX_STATUS.FAM_STAT_ID`,
  **scoped to the same `PAT_ENC_CSN_ID`**. Verified 73/73 rows match. This is how you attach "Ovarian cancer"
  to "Mother (Alive)". (Discovery missed this join — it's the spine of family hx.) (see §7 LINE child rows)
- **Pedigree (build the family tree):** `FAMILY_HX_STATUS.FAM_STAT_FATHER_ID` / `FAM_STAT_MOTHER_ID` →
  ...
  Mother rows; Mother's point at the maternal grandparents — a complete 3-generation graph. (see §25 pedigree)
- **Medical hx → diagnosis name:** `MEDICAL_HX.DX_ID = CLARITY_EDG.DX_ID`. Bare `DX_ID`; doc says
  `DX_ID_DX_NAME` (§5). Verified resolves to "Post concussion syndrome".
- **Surgical hx → procedure name:** `SURGICAL_HX.PROC_ID = CLARITY_EAP.PROC_ID`. Bare `PROC_ID`; doc says
  `PROC_ID_PROC_NAME` (§5). Verified `42500` → "WISDOM TOOTH EXTRACTION". (`CLARITY_EAP` = procedure/EAP
  master — *not* `CLARITY_EDG`.)
- **Which sections were reviewed at a visit:** `PAT_HX_REV_TYPE.PAT_ENC_CSN_ID` (= the history contact CSN);
  `HX_REVIEWED_TYPE_C_NAME` labels each reviewed section. Lines up 1:1 with the snapshot CSNs.

(see §6 base+supplement, §13 `_C_NAME` categories, §19 per-encounter re-snapshot, §17 `_HX` two meanings)
...
1. **The key CSN is a "history contact," not the visit — and `HX_LNK_ENC_CSN` is the visit.**
   *Observe:* `SOCIAL_HX.PAT_ENC_CSN_ID = 724623985` but `HX_LNK_ENC_CSN = 720803470`; the two are never
   ...
   history contact is the `.01`/`.02` same-day fraction (§10). Both are real `PAT_ENC` rows sharing the day,
...
2. **Per-encounter re-snapshot inflates every count (§19).** *Observe:* `FAMILY_HX` has 73 rows but the
...
3. **Conditions are stored differently in each history table.** *Observe:* `FAMILY_HX` names a condition with
   ...
   **EDG code**) and `SURGICAL_HX` uses `PROC_ID = 42500` (an **EAP procedure code**). *Why:* family history is
   ...
   from the inline `_C_NAME` (no `ZC_` table ships, §13); join `MEDICAL_HX.DX_ID → CLARITY_EDG` and
...
4. **`SOCIAL_HX` is one wide row; the others are LINE-tall.** *Observe:* `SOCIAL_HX` = exactly 1 row × 91
   ...
   (§6 shape, no supplements needed here); family/ADL are repeating sub-lists, so they're `LINE` child rows
   (§7). *Handle:* read `SOCIAL_HX` by column; reassemble the others by `ORDER BY CAST(LINE AS INT)`.
...
6. **The coded value and the free text disagree on purpose (§14).** *Observe:* `ALCOHOL_USE_C_NAME = "Yes"`
...
-- 5. Which history sections were reviewed, per visit (drives the re-snapshot)
SELECT PAT_ENC_CSN_ID, GROUP_CONCAT(DISTINCT HX_REVIEWED_TYPE_C_NAME) AS sections_reviewed
FROM PAT_HX_REV_TYPE GROUP BY PAT_ENC_CSN_ID;
```
...
- **No `ZC_` tables ship** (genre variation, §13), so the raw integer codes behind `MEDICAL_HX_C_NAME`,
